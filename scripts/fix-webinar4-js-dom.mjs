// fix-webinar4-js-dom.mjs
// Rebuilds all 23 steps of content/lessons/04-js-dom with:
//   - Full HTML context (final index.html from webinar 3 + script tag)
//   - Full CSS context (final style.css from webinars 2+3)
//   - Cumulative, syntactically correct js:start / js:solution for every step
// Run: node scripts/fix-webinar4-js-dom.mjs

import { writeFileSync, readFileSync } from 'fs';

const DIR = 'content/lessons/04-js-dom';

// ─── BASE ASSETS ─────────────────────────────────────────────────────────────

// HTML WITHOUT <script> tag (used as html:start for step 01)
const HTML_NO_SCRIPT = `<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
...
// HTML WITH <script> tag (used from step 01 solution onwards)
const HTML_WITH_SCRIPT = HTML_NO_SCRIPT.replace(
  '  <link rel="icon" type="image/svg+xml" href="img/logo.svg">\n</head>',
...
// CSS WITH .nav-link.active (added in step 11)
const CSS_WITH_ACTIVE = CSS_BASE + `
.nav-link.active {
  color: #007bff;
  background-color: #eaf2ff;
}`;

// ─── CUMULATIVE JS STATES ─────────────────────────────────────────────────────

// Helper — full showNotification function (complete, after step 10)
const FN_SHOW_NOTIFICATION = `// Функция всплывающих уведомлений (Toast) справа внизу
...
// Helper — full initNavigation function (complete, after step 17)
const FN_INIT_NAVIGATION = `// Функция подсветки активного пункта навигации
function initNavigation() {
  ...
  navLinks.forEach(link => {
    ...
    if ((current.endsWith('index.html') || current.endsWith('/') || current === '') && (href === 'index.html' || href === '../index.html')) {
      ...
      link.classList.add('active');
    } else if (href && href !== 'index.html' && href !== '../index.html' && !href.includes('catalog.html') && !href.includes('room-details.html') && current === href.split('/').pop()) {
      link.classList.add('active');
    }
  });
}`;

// Helper — full updateAuthNav function (complete, after step 23)
const FN_UPDATE_AUTH_NAV = `// Функция обновления навигации в зависимости от авторизации
function updateAuthNav() {
  const currentUser = localStorage.getItem('currentUser');
  const authNavBtn = document.getElementById('auth-nav-btn');
  const myBookingsNavItem = document.getElementById('my-bookings-nav');
  if (currentUser) {
...
// ─── STEP DEFINITIONS ─────────────────────────────────────────────────────────
// Each entry: { file, highlight, htmlStart, htmlSolution, cssStart, cssSolution, jsStart, jsSolution }
// null = don't emit that block

const STEPS = [
  // ── 01: create-js-file ─────────────────────────────────────────────────────
  {
    file: '01-create-js-file.md',
    highlight: 'html',
    htmlStart: HTML_NO_SCRIPT,
    htmlSolution: HTML_WITH_SCRIPT,
    cssStart: null, cssSolution: null,
    jsStart: null, jsSolution: null,
  },

  // ── 02: dom-content-loaded ─────────────────────────────────────────────────
  {
    file: '02-dom-content-loaded.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_BASE, cssSolution: null,
    jsStart: ``,
    jsSolution:
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── 03: toast-function-skeleton ────────────────────────────────────────────
  {
    file: '03-toast-function-skeleton.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_BASE, cssSolution: null,
    jsStart:
...
function showNotification(message, type = 'success') {
  ...
  // ── 04: toast-get-container ────────────────────────────────────────────────
  {
    file: '04-toast-get-container.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_BASE, cssSolution: null,
    jsStart:
...
function showNotification(message, type = 'success') {
  ...
  // ── 05: toast-create-container ─────────────────────────────────────────────
  {
    file: '05-toast-create-container.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_BASE, cssSolution: null,
    jsStart:
...
function showNotification(message, type = 'success') {
  ...
  // ── 06: toast-create-element ───────────────────────────────────────────────
  {
    file: '06-toast-create-element.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_BASE, cssSolution: null,
    jsStart:
...
function showNotification(message, type = 'success') {
  ...
  // ── 08: toast-timeout-skeleton ─────────────────────────────────────────────
  {
    file: '08-toast-timeout-skeleton.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_BASE, cssSolution: null,
    jsStart:
...
function showNotification(message, type = 'success') {
  ...
  // ── 09: toast-animation-out ────────────────────────────────────────────────
  {
    file: '09-toast-animation-out.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_BASE, cssSolution: null,
    jsStart:
...
${FN_SHOW_NOTIFICATION}`,
  },

  // ── 11: nav-css-active (CSS step) ─────────────────────────────────────────
  {
    file: '11-nav-css-active.md',
    highlight: 'css',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_BASE,
    cssSolution: CSS_WITH_ACTIVE,
    jsStart:
`// СмартОфис — Скрипт веб-приложения (Вебинар 4)
document.addEventListener('DOMContentLoaded', () => {
...
${FN_SHOW_NOTIFICATION}`,
  ...
  // ── 12: init-nav-function ──────────────────────────────────────────────────
  {
    file: '12-init-nav-function.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_WITH_ACTIVE, cssSolution: null,
    jsStart:
`// СмартОфис — Скрипт веб-приложения (Вебинар 4)
document.addEventListener('DOMContentLoaded', () => {
...
function initNavigation() {
  ...
  // ── 15: nav-highlight-index ────────────────────────────────────────────────
  {
    file: '15-nav-highlight-index.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_WITH_ACTIVE, cssSolution: null,
    jsStart:
...
function initNavigation() {
  ...
  // ── 16: nav-highlight-catalog ──────────────────────────────────────────────
  {
    file: '16-nav-highlight-catalog.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_WITH_ACTIVE, cssSolution: null,
    jsStart:
...
function initNavigation() {
  ...
  // ── 17: nav-highlight-others ───────────────────────────────────────────────
  {
    file: '17-nav-highlight-others.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_WITH_ACTIVE, cssSolution: null,
    jsStart:
...
${FN_INIT_NAVIGATION}`,
  ...
  // ── 18: auth-nav-function ──────────────────────────────────────────────────
  {
    file: '18-auth-nav-function.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_WITH_ACTIVE, cssSolution: null,
    jsStart:
...
// Функция обновления навигации в зависимости от авторизации
function updateAuthNav() {
  
}`,
  },

  // ── 19: local-storage-check ────────────────────────────────────────────────
  {
    file: '19-local-storage-check.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_WITH_ACTIVE, cssSolution: null,
    jsStart:
...
function updateAuthNav() {
  ...
  // ── 20: auth-logged-in-ui ──────────────────────────────────────────────────
  {
    file: '20-auth-logged-in-ui.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_WITH_ACTIVE, cssSolution: null,
    jsStart:
...
function updateAuthNav() {
  ...
  // ── 21: auth-logout-event ──────────────────────────────────────────────────
  {
    file: '21-auth-logout-event.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_WITH_ACTIVE, cssSolution: null,
    jsStart:
...
function updateAuthNav() {
  ...
  // ── 22: auth-logout-redirect ───────────────────────────────────────────────
  {
    file: '22-auth-logout-redirect.md',
    highlight: 'js',
    htmlStart: HTML_WITH_SCRIPT, htmlSolution: null,
    cssStart: CSS_WITH_ACTIVE, cssSolution: null,
    jsStart:
...
function buildStep(step, existingContent) {
  const { meta, body } = parseFrontmatter(existingContent);
  const explanation = extractExplanation(body);

  const blocks = [];

  if (step.htmlStart !== null) blocks.push(buildCodeBlock('html', 'start', step.htmlStart));
  if (step.htmlSolution !== null) blocks.push(buildCodeBlock('html', 'solution', step.htmlSolution));
  if (step.cssStart !== null) blocks.push(buildCodeBlock('css', 'start', step.cssStart));
  if (step.cssSolution !== null) blocks.push(buildCodeBlock('css', 'solution', step.cssSolution));
  if (step.jsStart !== null) blocks.push(buildCodeBlock('js', 'start', step.jsStart));
  if (step.jsSolution !== null) blocks.push(buildCodeBlock('js', 'solution', step.jsSolution));
...
for (const step of STEPS) {
  ...
  const newContent = buildStep(step, existing);
  writeFileSync(filePath, newContent, 'utf-8');
  console.log(`  ✅ Updated: ${step.file}`);
  updated++;
}
