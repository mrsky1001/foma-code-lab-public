...
Главная страница имеет несколько URL: `/`, `/index.html`, `../index.html` (из подпапок).

Проверяем все варианты через `||` (ИЛИ).

`classList.add('active')` — добавляет класс `active` к ссылке.

## 🛠 Задание

Внутри forEach после `classList.remove` добавьте:
```js
// Проверяем: если мы на главной странице (index.html или корень /), подсвечиваем ссылку на главную
if ((current.endsWith('index.html') || current.endsWith('/') || current === '') &&
    (href === 'index.html' || href === '../index.html')) {
  link.classList.add('active'); // добавляем класс активности ссылке главной
}
```
