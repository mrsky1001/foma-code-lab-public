...
Создаём сам Toast — маленькую карточку уведомления:

- `toast.className = 'toast toast-' + type` — два класса: базовый `toast` и цветовой `toast-success` / `toast-danger`
- `toast.textContent = message` — текст уведомления (безопасно, без XSS)

## 🛠 Задание

После блока `if (!container)` добавьте:
```js
const toast = document.createElement('div'); // создаём новый div в памяти (ещё не в DOM)
toast.className = 'toast toast-' + type; // два класса: базовый + цветовой (toast-success / toast-danger)
toast.textContent = message; // безопасно задаём текст (textContent не позволяет XSS-атаки)
```
