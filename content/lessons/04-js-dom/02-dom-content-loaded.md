...
Событие `DOMContentLoaded` срабатывает, когда браузер полностью построил HTML-дерево.

`document.addEventListener('DOMContentLoaded', () => { ... })` — всё что внутри стрелочной функции выполнится **после** загрузки страницы.

Это безопаснее чем писать код вне события: нет риска что JS обратится к элементу, которого ещё нет в DOM.

## 🛠 Задание

В `js/main.js` добавьте обёртку:
```js
// DOMContentLoaded — ждём пока браузер построит DOM-дерево из HTML
// Весь наш код внутри — он запустится только после загрузки страницы
