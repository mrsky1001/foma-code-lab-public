...
При клике на «Выйти» — удаляем пользователя из localStorage и показываем уведомление.

`e.preventDefault()` — отменяем переход по ссылке `href="#"`.
`localStorage.removeItem('currentUser')` — удаляем запись о входе.

## 🛠 Задание

Внутри блока `if (authNavBtn)` добавьте обработчик клика:
```js
// Обработчик нажатия на кнопку «Выйти»
authNavBtn.onclick = (e) => {
  e.preventDefault();                               // отменяем стандартный переход по ссылке
  localStorage.removeItem('currentUser');           // удаляем сессию пользователя из браузера
  showNotification('Вы вышли из системы', 'info');  // показываем всплывающее уведомление
};
```
