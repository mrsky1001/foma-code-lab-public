...
Если `currentUser` не null — пользователь вошёл. Меняем кнопку «Войти» на «Выйти».

`authNavBtn.textContent = 'Выйти'` — меняем текст кнопки.
`authNavBtn.href = '#'` — убираем переход по клику (обработаем через onclick).

## 🛠 Задание

Добавьте блок для авторизованного пользователя:
```js
// Если в localStorage есть данные пользователя — меняем кнопку на «Выйти»
if (currentUser) {
  if (authNavBtn) {
    authNavBtn.textContent = 'Выйти'; // меняем текст кнопки
    authNavBtn.href = '#';            // отменяем переход по ссылке
  }
}
```
