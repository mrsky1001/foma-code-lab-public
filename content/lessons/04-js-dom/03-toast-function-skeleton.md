...
Функция — многократно используемый блок кода. Вместо `alert()` создадим красивое всплывающее уведомление (Toast).

Имя `showNotification` — говорящее: глагол + существительное. Параметр `type = 'success'` — значение по умолчанию.

Функция определяется вне `DOMContentLoaded` — так её можно вызывать из любого места.

## 🛠 Задание

После блока `DOMContentLoaded` добавьте пустую функцию:
```js
// showNotification — функция для красивых всплывающих уведомлений вместо alert()
// message — текст уведомления; type — тип ('success' по умолчанию)
