...
Каталог включает страницы `catalog.html` и `room-details.html` (детальная страница — дочерняя от каталога).

`href.includes('catalog.html')` — проверяет есть ли подстрока. Работает и для относительных путей.

Паттерн: `else if` — проверяем только если предыдущее условие не сработало.

## 🛠 Задание

После блока index.html добавьте:
```js
// Если открыт каталог или детальная страница комнаты — подсвечиваем пункт меню «Каталог»
