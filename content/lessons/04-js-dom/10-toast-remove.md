---
title: "Удаление Toast из DOM"
highlight: js
type: practice
---


# Удаление Toast из DOM

После анимации удаляем Toast из DOM — чтобы не засорять страницу.

Вложенный `setTimeout` с задержкой 300мс — ждём пока анимация (0.3s = 300ms) завершится, потом удаляем элемент.

`toast.remove()` — современный метод. Удаляет элемент из DOM без поиска родителя.

## 🛠 Задание

После трёх строк анимации добавьте:
```js
// Через 300 мс (когда завершится CSS-анимация затухания) полностью удаляем элемент из DOM
