...
Если контейнера нет — создаём его через JS:

- `document.createElement('div')` — создаёт новый тег `<div>` в памяти
- `.id = 'toastContainer'` — устанавливает атрибут id
- `.className = 'toast-container'` — устанавливает класс CSS
- `document.body.appendChild(container)` — добавляет div в конец `<body>`

## 🛠 Задание

После строки с `getElementById` добавьте:
```js
// Если контейнер для уведомлений ещё не существует на странице — создаём его
if (!container) {
  container = document.createElement('div');       // создаём тег div
  container.id = 'toastContainer';                 // задаём id для быстрого поиска
  container.className = 'toast-container';         // класс с фиксированным позиционированием в углу
  document.body.appendChild(container);            // добавляем в конец <body>
}
```
