...
Файл `js/main.js` уже создан. Осталось подключить его к HTML через тег `<script>`.

Атрибут `defer` — ключевой: браузер скачает скрипт, но выполнит его только **после** полной загрузки HTML. Без него JS попытается найти элементы, которых ещё нет на странице.

## 🛠 Задание

В `<head>` после `<link rel="icon">` добавьте:
```html
