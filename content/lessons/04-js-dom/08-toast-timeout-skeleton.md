...
`setTimeout(callback, delay)` — выполняет функцию через `delay` миллисекунд (3500 = 3.5 секунды).

Это асинхронный код: браузер не ждёт — он продолжает работать, а через 3.5 сек выполняет callback.

Пока внутри setTimeout пусто — заполним следующим шагом.

## 🛠 Задание

Добавьте таймер:
```js
// Таймер: через 3500 мс (3.5 секунды) запускаем исчезновение уведомления
setTimeout(() => {
