...
# UI неавторизованного пользователя

Блок `else` — пользователь не вошёл. Возвращаем кнопку «Войти» с правильным href.

`authNavBtn.onclick = null` — убираем ранее установленный обработчик выхода.

**Модуль 4 завершён!** Система уведомлений Toast работает, навигация подсвечивает активный пункт, кнопка входа/выхода меняется динамически.

## 🛠 Задание

После блока `if (currentUser)` добавьте `else`:
```js
// Если пользователь НЕ авторизован — возвращаем кнопку «Войти» со ссылкой на страницу логина
} else {
  if (authNavBtn) {
    authNavBtn.textContent = 'Войти';
    const isPages = window.location.pathname.includes('/pages/');
    authNavBtn.href = isPages ? 'login.html' : 'pages/login.html'; // правильный относительный путь
    authNavBtn.onclick = null;                                     // очищаем обработчик выхода
  }
}
```
