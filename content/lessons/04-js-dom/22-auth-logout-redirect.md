...
После выхода — перенаправляем на главную страницу через 1 секунду.

`window.location.pathname.includes('/pages/')` — определяем находимся ли мы в подпапке `pages/`. Это важно для правильного формирования пути.

`setTimeout` — даём время Toast-уведомлению показаться.

## 🛠 Задание

После строки showNotification добавьте:
```js
const isPages = window.location.pathname.includes('/pages/'); // проверяем, находимся ли во вложенной папке pages
setTimeout(() => {
  window.location.href = isPages ? '../index.html' : 'index.html'; // перенаправляем на главную страницу
}, 1000);
```
