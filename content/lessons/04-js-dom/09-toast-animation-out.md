...
Плавное исчезновение через CSS-переходы из JS:

`toast.style.transition` — устанавливаем CSS transition прямо из JS. Это аналог `transition: opacity 0.3s` в CSS.

`opacity = '0'` и `transform = 'translateY(10px)'` — Toast плавно тает и уходит вниз.

## 🛠 Задание

Внутри setTimeout добавьте:
```js
toast.style.transition = 'opacity 0.3s ease, transform 0.3s ease'; // плавная анимация исчезновения
toast.style.opacity = '0';                                         // делаем плавно прозрачным
toast.style.transform = 'translateY(10px)';                        // сдвигаем слегка вниз
```
