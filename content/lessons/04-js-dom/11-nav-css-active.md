---
title: "CSS активного пункта меню"
highlight: css
type: practice
---

# CSS активного пункта меню

Пункт меню текущей страницы должен быть визуально выделен — пользователь понимает где он находится.

Класс `.active` будем добавлять через JS (следующие шаги). CSS здесь просто описывает внешний вид.

Синий цвет + голубой фон — те же что при :hover, но постоянные.

## 🛠 Задание

В `css/style.css` добавьте:
```css
.nav-link.active {
  color: #007bff;            /* синий цвет текста для активного пункта меню */
  background-color: #eaf2ff; /* светло-голубая подложка */
}
```
