---
title: "Добавление Toast в DOM"
highlight: js
type: practice
---


# Добавление Toast в DOM

`container.appendChild(toast)` — добавляет Toast в контейнер. Теперь уведомление видно на странице.

Порядок важен: сначала создаём элемент, настраиваем его, потом добавляем в DOM. Это быстрее чем добавлять и потом менять.

## 🛠 Задание

Добавьте строку:
```js
// Добавляем созданное уведомление внутрь контейнера (оно появляется на экране)
