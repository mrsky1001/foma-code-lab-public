...
Мы запишем логин пользователя в `localStorage` под ключом `currentUser`. Наша функция `updateAuthNav()` (из Модуля 4) уже умеет читать этот ключ и менять кнопку "Войти" на "Выйти"!
...
4. Редирект на пока еще несуществующую страницу `my-bookings.html` (мы создадим её в будущих модулях).
