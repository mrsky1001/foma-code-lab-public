...
Как мы обсуждали в 4-м модуле, самый безопасный момент для запуска любых скриптов, работающих с HTML — это событие `DOMContentLoaded` (когда браузер полностью скачал и построил HTML структуру).

## 🛠 Задание
В самом верху файла `main.js` найдите слушатель события `DOMContentLoaded` и добавьте внутрь него вызов обеих новых функций.

На этом Модуль 6 окончен! Вы научились круто валидировать формы. Попробуйте теперь зайти на страницу регистрации и нажать кнопку отправки, оставив поля пустыми!
