---
title: "CSS изображения карточки"
highlight: css
type: practice
---

# CSS изображения карточки

`object-fit: cover` — ключевое свойство для красивых фото:
- Масштабирует картинку с сохранением пропорций
- Заполняет весь блок без сплющивания
- Обрезает лишнее по краям

Без этого свойства вытянутые или широкие фото будут выглядеть коряво.

## 🛠 Задание

Добавьте стили для блока и картинки:
```css
.card-img-wrap {
  ...
  background-color: #f0f4f8; /* серый фон пока картинка не загрузилась */
}

.card-img {
  width: 100%;               /* заполнить всю ширину блока */
  height: 100%;              /* заполнить всю высоту блока */
  object-fit: cover;         /* масштабировать с обрезкой, без искажений */
  object-position: center;   /* центрировать картинку внутри блока */
  display: block;            /* убрать строчный отступ снизу у img */
}
```
