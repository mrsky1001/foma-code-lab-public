...
В `div.card-btns` добавляем две кнопки:
1. `.btn-icon` — квадратная с SVG-иконкой (переход к деталям)
2. `.btn.btn-primary` — синяя «Забронировать»

SVG-иконка рисуется прямо в HTML — это векторная графика, не нужен отдельный файл.

`.card-btns { display: flex; gap: 8px }` — кнопки рядом.

## 🛠 Задание

В `div.card-btns` добавьте кнопки и CSS:

HTML:
```html
<!-- Кнопка-иконка «Подробнее» (открывает страницу комнаты) -->
...
```css
.card-btns {
  display: flex;         /* размещаем кнопки в один ряд */
  align-items: center;   /* выравнивание по центру по вертикали */
  gap: 8px;              /* отступ 8px между кнопками */
}
```
