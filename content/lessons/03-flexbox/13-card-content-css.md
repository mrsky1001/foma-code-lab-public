---
title: "CSS контента карточки"
highlight: css
type: practice
---

# CSS контента карточки

`.card-content { flex: 1 }` — важный трюк:

Когда карточки во Flexbox-контейнере разной высоты (разное количество пунктов в списке), `flex: 1` растягивает card-content на всю доступную высоту. Это позволит прижать card-footer к низу карточки.

## 🛠 Задание

Добавьте стили блока контента:
```css
.card-content {
  padding: 15px;         /* внутренние отступы контента карточки */
  display: flex;         /* flex-контейнер для вертикальной компоновки */
  flex-direction: column; /* заголовок → список → footer сверху вниз */
  flex: 1;               /* растянуть до конца карточки — footer прижат вниз */
}

.card-title {
  font-size: 18px;     /* крупное название комнаты */
  font-weight: 700;    /* жирный заголовок */
  margin-bottom: 10px; /* отступ до списка оснащения */
}
```
