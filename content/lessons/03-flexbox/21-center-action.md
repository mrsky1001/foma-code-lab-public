---
title: "Кнопка «Больше офисов»"
highlight: html
type: practice
---

# Кнопка «Больше офисов»

Финальный штрих — кнопка ведущая в каталог всех комнат.

`.center-action { text-align: center }` — самый простой способ центрировать inline/inline-block элемент (каковым является `.btn`).

Модуль 3 завершён! У нас теперь полноценная секция с карточками, выровненными через Flexbox.

## 🛠 Задание

После `</div>` (rooms-grid) добавьте + CSS:

HTML:
```html
<!-- Блок с кнопкой перехода в полный каталог (по центру секции) -->
...
```css
.center-action {
  text-align: center;  /* центрируем строчно-блочную кнопку */
  margin-top: 20px;    /* отступ 20px сверху от сетки карточек */
}
```

**Модуль 3 завершён! Карточки с Flexbox — это важнейший инструмент в современной верстке!**
