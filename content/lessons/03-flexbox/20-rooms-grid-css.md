---
title: "CSS сетки: Flexbox для карточек"
highlight: css
type: practice
---

# CSS сетки: Flexbox для карточек

Главный момент модуля! Flexbox выстраивает карточки в красивый ряд:

- `display: flex` — Flexbox-контейнер
- `flex-wrap: wrap` — если карточки не помещаются — переносятся на новую строку
- `gap: 20px` — промежутки между карточками

Математика: 3 × 373px + 2 × 20px = 1159px < 1200px (ширина .container) ✅

## 🛠 Задание

Добавьте CSS для grid-контейнера:
```css
.rooms-grid {
  display: flex;       /* Flexbox-контейнер для карточек */
  flex-wrap: wrap;     /* перенос карточек на новую строку если не помещаются */
  gap: 20px;           /* расстояние 20px между карточками */
  margin-top: 30px;    /* отступ от заголовка секции */
  margin-bottom: 30px; /* отступ до следующего элемента */
}
```

Три карточки выстроятся в ряд!
