...
Изображение — это ссылка (`<a>`) с картинкой внутри. При клике на фото — переход на детальную страницу.

Атрибут `onerror` — JS-обработчик: если картинка не загрузилась, подставляем заглушку `no-image.svg`.

`class="card-img"` — стили применим в следующем шаге.

## 🛠 Задание

В `div.card-img-wrap` добавьте:
```html
<!-- Ссылка-обёртка ведёт на детальную страницу комнаты по её id -->
<a href="pages/room-details.html?id=focus-1">
  <!-- onerror подставит заглушку no-image.svg, если картинка не загрузится -->
