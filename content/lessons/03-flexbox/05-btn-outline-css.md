...
Кнопка с обводкой (outline) — для вторичных действий. Прозрачный фон, синяя рамка.

При наведении (`:hover`) заполняется синим — инверсия цветов.

Используем для кнопки «Больше офисов» — она менее важна чем «Забронировать».

## 🛠 Задание

Добавьте кнопку с обводкой:
```css
.btn-outline {
  background-color: transparent;  /* прозрачный фон — кнопка только с обводкой */
  border-color: #007bff;          /* синяя рамка */
  color: #007bff;                 /* синий текст */
}

.btn-outline:hover {
  background-color: #007bff;  /* при наведении — заполняется синим */
  color: #ffffff;             /* текст становится белым (инверсия) */
}
```
