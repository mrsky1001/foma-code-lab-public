...
При наведении (`:hover`) чуть темнеем фон — даём пользователю визуальный отклик.

Принцип: **один класс — одна идея**. `.btn-primary` только о цвете, не о размере.

## 🛠 Задание

Добавьте модификатор для синей кнопки:
```css
.btn-primary {
  background-color: #007bff;  /* синий фон — главная кнопка призыва к действию */
