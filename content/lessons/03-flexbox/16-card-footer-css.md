---
title: "CSS подвала карточки"
highlight: css
type: practice
---

# CSS подвала карточки

`justify-content: space-between` разносит цену и кнопки к краям.

`border-top: 1px solid #eeeeee` — лёгкий разделитель между списком оснащения и ценой.

## 🛠 Задание

Добавьте стили card-footer:
```css
.card-footer {
  display: flex;                  /* цена и кнопки в строку */
  justify-content: space-between; /* цена слева, кнопки справа */
  align-items: center;            /* выровнять по центру вертикально */
  border-top: 1px solid #eeeeee; /* тонкий разделитель над подвалом */
  padding-top: 10px;             /* отступ от разделителя */
  gap: 10px;                     /* промежуток между ценой и кнопками */
}
```
