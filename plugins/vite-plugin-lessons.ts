...
function parseMdStep(content: string, filePath: string): LessonStep {
  ...
  const solutionCode: CodeFiles = hasSolutionBlock
    ? {
        html: (codeBlocks['html:solution'] && codeBlocks['html:solution'].trim())
          ? codeBlocks['html:solution']
          : startCode.html,
        css: (codeBlocks['css:solution'] && codeBlocks['css:solution'].trim())
          ? codeBlocks['css:solution']
          : startCode.css,
        js: (codeBlocks['js:solution'] && codeBlocks['js:solution'].trim())
          ? codeBlocks['js:solution']
          : startCode.js,
      }
    : { ...startCode };
  ...
  return {
    ...
    type: meta.type as LessonStep['type'] | undefined,
  };
}
    ...
    configureServer(server) {
      ...
      if (fs.existsSync(contentDir)) {
        ...
        const handleFile = (file: string) => {
          ...
          if (normFile.endsWith('.md') || normFile.endsWith('.yml') || normFile.endsWith('.yaml')) {
        ...
        server.watcher.on('unlink', handleFile);
      }
    },
  };
}
